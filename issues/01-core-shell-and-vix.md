...
- [ ] Project directory structure is created with `scripts/update_data.py` and frontend files `index.html`, `style.css`, and `app.js`.
- [ ] Running the Python script retrieves historical VIX data (`^VIX` index via `yfinance`) and outputs a structured `data/market_data.json` containing the data points and a `last_updated` timestamp.
